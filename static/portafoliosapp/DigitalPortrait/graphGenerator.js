...
function regionWeight(nx, ny) {
  ...
  const face = nx > 0.22 && nx < 0.78 && ny > 0.18 && ny < 0.56 ? 0.06 : 0;
  const neck = nx > 0.28 && nx < 0.72 && ny > 0.52 && ny < 0.66 ? 0.12 : 0;
  const jacket = ny > 0.62 ? 0.24 : 0;
  const shoulder = ny > 0.7 && (nx < 0.36 || nx > 0.64) ? 0.18 : 0;
  const center = Math.max(0, 0.09 - Math.abs(nx - 0.5) * 0.14);
  const clothingCenter = ny > 0.58 ? Math.max(0, 0.16 - Math.abs(nx - 0.5) * 0.2) : 0;
  return cap + visor + capSide + face + neck + jacket + shoulder + center + clothingCenter;
}

function graphRegion(nx, ny, type) {
  if (type === "cap" || ny < 0.36) return "cap";
...
export function generateGraph(sample, options = {}) {
  ...
  const maxNodes = options.maxNodes || (window.innerWidth < 760 ? 1150 : 2350);
  ...
  for (let y = 3; y < height - 3; y += stride) {
    for (let x = 3; x < width - 3; x += stride) {
      ...
      const flatChance = pixel.a > 0.72 ? (capZone ? 0.16 : clothingZone ? 0.15 : faceZone ? 0.052 : 0.09) : 0.04;
      const keep = density > (capZone || clothingZone ? 0.095 : 0.12) || random() < flatChance + density * (capZone || clothingZone ? 0.62 : 0.42);
      ...
      const type = capZone && luminance < 0.55
        ? "cap"
        : alphaEdge > 0 ? "silhouette" : edge > 0.18 || contrast > 0.13 ? "feature" : luminance < 0.42 ? "dark" : "surface";
      ...
      candidates.push({
        ...
        radius: 0.42 + density * 1.55 + (type === "surface" ? random() * 0.36 : type === "cap" ? random() * 0.68 : random() * 0.82),
        opacity: 0.24 + density * 0.62 + (type === "surface" ? random() * 0.08 : random() * 0.18),
        color: colorFromPixel(pixel, edge, random),
        driftX: (random() - 0.5) * (type === "surface" ? 10 : type === "cap" ? 13 : 18),
        driftY: (random() - 0.5) * (type === "surface" ? 10 : type === "cap" ? 13 : 18),
  ...
  nodes.forEach((node, index) => {
    ...
    neighbors
      ...
      .forEach((neighbor) => {
        ...
        const bothSurface = node.type === "surface" && target.type === "surface";
